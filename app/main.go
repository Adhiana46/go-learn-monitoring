...
func (h registerDevicesHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case "GET":
		getDevices(w, r, h.metrics)
	case "POST":
		createDevice(w, r, h.metrics)
	default:
...
func (mdh manageDevicesHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case "PUT":
		log.Println("Handling PUT /devices/{id}")
